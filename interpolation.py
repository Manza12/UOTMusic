...
def conic_interp(a0, a1, L, t):
    ...
    b = np.arctan((1-t) * np.sqrt(a0) + t * np.cos(L) * np.sqrt(a1), t * np.sin(L) * np.sqrt(a1))
...
def conic_interp_measures(xs, ys, p, q, ts, lam=1e-2, tol=1e-6, thr=1e-2, straight=True):
    ...
    for i in range(np.size(p)):
        for j in range(np.size(q)):
            if M[i, j]:
                ...
                bs = np.arctan(ts * np.sin(L[i, j]) * np.sqrt(gamma_b[i, j]), (1. - ts) * np.sqrt(gamma_a[i, j]) + ts * np.cos(L[i, j]) * np.sqrt(gamma_b[i, j])) / L[i, j]
    ...
    for i in range(np.size(p)):
        if np.abs(np.sum(gamma_a[i, :]) / p[i] - 1) > 1 / 2:  # mass disappears
            ...
            X = np.concatenate((X, np.transpose(xs[i] * np.ones(np.size(ts)))), axis=0)
    for j in range(np.size(q)):
...
if __name__ == '__main__':
    ...
    _u, _v, _gamma, _errs = scaling(_c, _p, _q, lam=1e-2, rho=1, tol=1e-4)
    ...
    A, X = conic_interp_measures(_xs, _ys, _p, _q, _ts, lam=1e-2, tol=1e-5, thr=1e-3)
